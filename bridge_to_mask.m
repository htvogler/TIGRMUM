...
%
% radius: corridor half-width. Pass the tube's own local width (e.g. half
% of a regionprops MinorAxisLength, or diamo/2 once available), not a
% small fixed value -- a corridor much thinner than the real tube creates
% a "dumbbell" pinch that visibly looks wrong (still-separate-looking
% "connections only 2-3px wide") and confuses the tip-detection/branch-
% removal logic downstream even when the two pieces are technically one
% connected component (verified on HV198_1_16 frame 3224: a thin corridor
% left the traced centerline stopping at the pinch and the tip getting
% placed at that junction instead of the real, further-out tip).
[D, IDX] = bwdist(base);
frag_idx = find(frag);
[~, rel] = min(D(frag_idx));
